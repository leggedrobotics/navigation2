  ...
  const geometry_msgs::msg::PoseStamped & robot_pose) const
{
  ...
  // Once the final incoming segment is active and the base crosses the cusp
  // plane, retaining that segment can only drive it farther from the reversal.
  // Activate the outgoing segment so its opposite traversal direction can
  // bring the base back. The active-segment gate avoids releasing early on a
  // looping path that happens to cross the infinite cusp plane elsewhere.
  const size_t final_incoming_pose_count =
    inversion_forward_hold_distance_ > 0.0 ? 3u : 2u;
  if (longitudinal_error >= 0.0 &&
    global_plan_up_to_constraint_.poses.size() <= final_incoming_pose_count)
  {
    return true;
  }
