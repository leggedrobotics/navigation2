...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  if (current_path_.poses.size() >= 2) {
    ...
    // Use monotonic progress for remaining distance, but nearest-path error for tracking bounds.
    const auto path_progress_result = nav2_util::distance_from_path(
      current_path_, robot_pose_in_path_frame.pose, start_index_, params_->search_window);
    const auto path_tracking_result = nav2_util::distance_from_path(
      current_path_, robot_pose_in_path_frame.pose, 0, std::numeric_limits<double>::max());

    // Calculate heading error
    double heading_tracking_error = 0.0;
    if (path_tracking_result.closest_segment_index <
      current_path_.poses.size() - 1)
    {
      const auto & path_segment_start =
        current_path_.poses[path_tracking_result.closest_segment_index].pose;
      const auto & path_segment_end =
        current_path_.poses[path_tracking_result.closest_segment_index + 1].pose;
    ...
    tracking_feedback_msg->position_tracking_error = path_tracking_result.distance;
    tracking_feedback_msg->heading_tracking_error = heading_tracking_error;
    tracking_feedback_msg->current_path_index = path_progress_result.closest_segment_index;
    ...
    start_index_ = path_progress_result.closest_segment_index;
